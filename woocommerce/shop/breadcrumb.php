...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		if ( $this_category->parent != 0 ) {
			...
			echo $before . get_category_parents($parent_category, TRUE, $after . $delimiter . $before ) . $after;
		}

		echo $before . '<a class="current" href="' . esc_url(get_category_link($this_category->term_id)) . '">' . single_cat_title( '', false ) . '</a>' . $after;
		...
		echo $before . '<a class="current" href="' . get_term_link( $queried_object->slug, "product_cat" ) . '">' . esc_html( $queried_object->name ) . '</a>' . $after;

	} elseif ( is_tax('product_tag') ) {

		$queried_object = $wp_query->get_queried_object();
		echo $prepend . $before . '<a class="current" href="' . get_term_link( $queried_object->slug, $queried_object->name ) . '">' . __( 'Products tagged &ldquo;', 'woocommerce' ) . $queried_object->name . '&rdquo;</a>' . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="#">' . $_name . '</a>' . $after;
		...
		if ( get_post_type() == 'product' ) {
			...
			echo $before . '<a class="current" href="' . get_permalink($post->ID) . '">' . get_the_title() . '</a>' . $after;
	...
	if ( get_query_var( 'paged' ) )
		echo $before .' <a href="#" class="current"><small>(' . __( 'page', 'woocommerce' ) . ' ' . get_query_var( 'paged' ) . ')</small></a>';
