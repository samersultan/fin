...
<div id="order_review" class="row">
	<div class="small-12 large-6 columns" id="review-order">
		<h3 id="order_review_heading"><?php _e( 'Your order', 'woocommerce' ); ?></h3>
		<table class="shop_table small-12 columns">
			<thead>
				<tr>
					...
					if (sizeof($woocommerce->cart->get_cart())>0) :
						foreach ($woocommerce->cart->get_cart() as $item_id => $values) :
							...
							if ($_product->exists() && $values['quantity']>0) :
								$attributes = $woocommerce->cart->get_item_data( $values );
								if($attributes != '') {
								...
								echo '
									<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $item_id ) ) . '">
										<td class="product-name"><strong class="product-quantity">' . $values['quantity'] . ' &times; </strong><a href="'. get_permalink( apply_filters('woocommerce_in_cart_product_id', $values['product_id'] ) ) . '">' . $_product->get_title() . '</a>' . $attributes . '</td>
	...
	<div class="small-12 large-6 columns" id="payment">
		<?php if ($woocommerce->cart->needs_payment()) : ?>
		<h3>Payment</h3>
		<ul class="payment_methods methods unstyled">
			<?php
				...
				if ( ! empty( $available_gateways ) ) {
					...
					foreach ( $available_gateways as $gateway ) {
						?>
						<li>
							<label for="payment_method_<?php echo $gateway->id; ?>"><input type="radio" id="payment_method_<?php echo $gateway->id; ?>" class="input-radio" name="payment_method" value="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( $gateway->chosen, true ); ?>> <?php echo $gateway->get_title(); ?> <?php echo $gateway->get_icon(); ?></label>
